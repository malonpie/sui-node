...
echo '-------------------------------------'
echo $(date)
SUIVER=$(curl --location --request POST https://fullnode.testnet.sui.io:443 \
...
echo 'Node ledger version : '$MYVER
...
echo '-------------------------------------'
echo $(date)
NOWSUIVER=$(curl --location --request POST https://fullnode.testnet.sui.io:443 \
--header 'Content-Type: application/json' \
--data-raw '{ "jsonrpc":"2.0", "method":"sui_getTotalTransactionNumber","id":1}' 2>/dev/null | jq .result)
